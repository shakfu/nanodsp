...
| `DaisySP/Source/PhysicalModeling/drip.cpp` | `noise_tick()` scales by `kRandFrac` instead of the hard-coded `1073741823.5f` / `1073741823.0f` | Those constants are glibc's `RAND_MAX/2`. Where `RAND_MAX` is 32767 -- MSVC, so every Windows build -- the whole range of `rand()` sits at the bottom of the assumed one and `noise_tick()` returns a constant -0.99997 rather than white noise. The excitation becomes DC and `gains{0,1,2}_ = fabsf(noise_tick())` latches at 1.0, driving the three `WUTR_RESON`=0.9985 resonators far past unity: `synthesis.drip` peaked at 28.7 against the limit of 10 in `tests/test_default_output.py`, which is what failed the Windows wheel build. `kRandFrac` is `1.f / RAND_MAX` from `dsp.h` and is how every other DaisySP noise source scales `rand()`; where `RAND_MAX` is 2147483647 the replacement is bit-identical to the original for all 2^31 draws, so no stored fingerprint moves. |
